...
interface Match { type: PiiType; start: number; end: number; value: string; }
...
export class WebAnonymizer {
  ...
  private counters: Record<PiiType, number> = { EMAIL:0, PHONE:0, IP:0, IBAN:0, NIN:0, BVN:0, CARD:0, NAME:0, CUSTOM:0 };
  private customExactTerms: string[] = [];
  private scope = 'browser-session';

  setCustomTerms(terms: string[]) { this.customExactTerms = terms ?? []; }
  clear() { this.byOriginal.clear(); this.byPlaceholder.clear(); Object.keys(this.counters).forEach(k => (this.counters as any)[k] = 0); }
  ...
  unmask(text: string): RestoreResult {
    const restored = text.replace(/\[\[LDB:([A-Z]+)_(\d+)\]\]/g, (full) => this.byPlaceholder.get(full) ?? full);
    return { restoredText: restored };
  }

  private findMatches(text: string): Match[] {
    const out: Match[] = [];
    const push = (re: RegExp, type: PiiType) => { re.lastIndex = 0; let m: RegExpExecArray|null;
      while ((m = re.exec(text))) out.push({ type, start: m.index, end: m.index + m[0].length, value: m[0] });
    };
    push(EMAIL_RE,'EMAIL'); push(PHONE_RE,'PHONE'); push(IP_RE,'IP'); push(IBAN_RE,'IBAN'); push(NIN_BVN_RE,'NIN'); push(CARD_RE,'CARD');
    for (const term of this.customExactTerms) {
      if (!term) continue;
      const esc = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const re = new RegExp(`\\b${esc}\\b`, 'gi'); let m: RegExpExecArray|null;
      while ((m = re.exec(text))) out.push({ type:'CUSTOM', start:m.index, end:m.index+m[0].length, value:m[0] });
    }
  ...
  private getPlaceholder(type: PiiType, original: string): string {
    ...
    this.byOriginal.set(key, ph); this.byPlaceholder.set(ph, original);
    return ph;
  }
}
